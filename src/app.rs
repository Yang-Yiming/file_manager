...
use crate::theme::{ThemeMode, ModernTheme};
use eframe::egui;
...
impl FileManagerApp {
    ...
    pub fn new() -> Self {
        ...
        let compact_mode = config.compact_mode;
    ...
    fn update_filter(&mut self) {
        ...
        if self.search_query != self.last_search_query {
            self.filtered_indices = self
                ...
                .enumerate()
                .filter(|(_, entry)| {
                    self.matches_search_query(entry)
                })
                .map(|(i, _)| i)
                .collect();
    ...
    fn matches_search_query(&self, entry: &FileEntry) -> bool {
        ...
        for part in query_parts {
            let found = if part.starts_with('#') {
                ...
                hash_tags.iter().any(|tag| tag.to_lowercase().contains(part))
            } else {
                // 普通搜索：文件名、昵称、描述
    ...
    fn render_import_export(&mut self, ui: &mut egui::Ui) {
        ...
        ui.label("批量操作:");
        ui.label("标签:");
        if ui.text_edit_singleline(&mut self.batch_tag_input).changed() && !self.batch_tag_input.is_empty() {
        ...
        if !self.selected_tags.is_empty() {
            ...
            ui.horizontal(|ui| {
                if ui.button("按选中标签过滤").clicked() {
                    let tag_queries: Vec<String> = self.selected_tags.iter()
                    ...
                    if self.compact_mode {
                        ...
                        ui.horizontal(|ui| {
                            ...
                            if !hash_tags.is_empty() {
                                ui.small(format!("🏷️{}", hash_tags.len()));
                            }
                        ...
                        ui.horizontal(|ui| {
                            ...
                            ui.vertical(|ui| {
                                ...
                                if let Some(nickname) = &entry_nickname {
                                    ...
                                    ui.small(egui::RichText::new(&entry_name).color(ModernTheme::weak_text_color(ui.ctx())));
                                } else {
                                    if ui.link(&entry_name).clicked() {
                                        self.open_path(&entry_path);
                                    }
                                ...
                                if let Some(desc) = &entry_description {
                                    ui.small(egui::RichText::new(desc).italics().color(ModernTheme::weak_text_color(ui.ctx())));
                                }
...
impl eframe::App for FileManagerApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::TopBottomPanel::top("top").show(ctx, |ui| {
            ui.horizontal(|ui| {
                ui.label(egui::RichText::new("🗂️ 文件管理器").strong());
                ui.separator();
                
                // 统一搜索框（支持文件名、标签和描述）
                ui.label("🔍");
                ui.add_sized([250.0, 20.0], egui::TextEdit::singleline(&mut self.search_query).hint_text("搜索文件、标签..."));
                if ui.ctx().input(|i| i.key_pressed(egui::Key::Enter)) || self.search_query != self.last_search_query {
                    self.force_update_filter();
                }
        ...
        // 侧边面板
        if self.show_add_dialog || self.show_tag_editor || self.show_settings || self.show_import_export || self.show_tag_manager {
            egui::SidePanel::right("side").width_range(250.0..=300.0).default_width(280.0).show(ctx, |ui| {
